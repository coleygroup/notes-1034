...
\begin{solution}
    ...
    \[
        ...
        \begin{pmatrix}C_A\\C_B\end{pmatrix}.
    \]
    The eigenvalues of a triangular matrix (upper- or lower-triangle) are just the diagonal elements; in this case, $-1\;\mathrm{sec}^{-1}$ and $-10\;\mathrm{sec}^{-1}$. The largest (magnitude) eigenvalue, corresponding to the shortest timescale, is smallest timescale, is
    $-10\;\mathrm{sec}^{-1}$.
...
\begin{solution}
    ...
    On a log-log plot, these power laws appear as lines, whose slopes are the exponents of the power laws. Therefore, the slope of the Crank-Nicholson line should be twice that of the forward Euler line, indicating that the blue dots represent Crank-Nicolson global truncation error.
\end{solution}

\begin{exercise} % 2020 Participation quiz L18
    Suppose we are solving a BVP using hte Shooting Method. If an initial guess $c$ gives a result $x(\tau; c) > x_f$, where $x(\tau) = x_f$ is the boundary condition for
    \[
...
\begin{exercise} % 2020 Participation quiz L19
    ...
    using
    \[
        x(t) = \sum_{i=1}^N c_i \cos \left(\frac{i\pi x}{t_f-t_0}\right),
    \]
    how many collocation points, \{\(t_i\)\}, do we need to choose?
\end{exercise}

\begin{solution}
    Let the state be \(M\)-dimensional. Then, there are \underline{\underline{\(M\cdot N\) unknowns}} \(c_i \in\mathbb{R}^M,\, i=1,\cdots, N\) in the truncated series expansion. Without any further information, we shouldn't assert that any of the BCs is implied by the choice of basis functions.
    ...
    \[
        ...
        g\left(x(t_0),x(t_1)\right) = 0                                       & \qquad\longrightarrow\qquad & M \text{ equations}\\[4pt]
              \cline{3-3}\\[-8pt]
