...
\begin{solution}
    \begin{enumerate}[label=(\alph*)]
        ...
        \item Samples are taken from $\mathcal{U}(-2,2)$, which doesn't
        cover all density of $R(x)$.

        \item The estimate of $N$ is just the average value of $f$ over
        the samples.  We need to multiply this average by the ``area'' of
        the samples.  We do this by taking the mean of $f(x)/P(x)$ instead
        of just $f(x)$.  Corrected code:
        \lstinputlisting{code/matlab/monte-carlo/corrected2.m}

        \item MCMC lets us sample points from an arbitrary distribution.
        In the previous two parts we used \texttt{rand} and \texttt{randn}
        to sample points from the uniform and Gaussian distributions.  We
        don't need MCMC to sample from these two distributions as we
        already have functions to do so.
