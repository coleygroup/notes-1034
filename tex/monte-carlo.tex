...
It is worth being precise about exactly what this dimension-independence means. Like big-O notation, which hides a constant prefactor while capturing only the dominant scaling exponent (recall the discussion of computational complexity earlier in the notes), Monte Carlo's $M^{-1/2}$ rate is dimension-independent only in the sense that the \emph{exponent} on $M$ does not change with $N$. The constant multiplying that rate typically grows substantially with dimension for realistic problems, which is why designing a good sampling or proposal distribution is still an essential consideration high-dimensional Monte Carlo, even if we have $M^{-1/2}$ scaling by construction.
...
\end{equation}
This ensures that $M q(\mathbf{x})$ forms an envelope over $p(\mathbf{x})$ (in the 1D example shown in \autoref{fig:rejection-overview}, the bound $M q(\mathbf{x})$ is shown as the black dotted line (a constant) since we are using a uniform proposal distribution). For each proposed sample $\mathbf{x}_i$ drawn from $q(\mathbf{x})$, we generate a uniform random number $u_i \in [0,1]$ and accept the sample if
\begin{equation}
...
\begin{equation}
    \pi(\mathbf{x}) = \lim_{M \to \infty} \Pr(\mathbf{x}_M) = \lim_{M \to \infty} \int_{\Omega} T(\mathbf{x}, \mathbf{x}_{M-1}) \Pr(\mathbf{x}_{M-1}) \, d\mathbf{x}_{M-1} = \int_{\Omega} T(\mathbf{x}, \mathbf{y}) \pi(\mathbf{y}) \, d\mathbf{y}
\end{equation}
...
The detailed balance is sufficient but not necessary for a limiting distribution to exist.

Modern MCMC methods, including non-reversible (``lifted'') samplers and, more practically relevant, Hamiltonian Monte Carlo (HMC), deliberately construct transitions that violate detailed balance. It is provable that well-designed non-reversible chains mix faster than any reversible chain using the same computational effort, which is part of why HMC and its relatives have become the default choice for sampling continuous, high-dimensional distributions in modern practice. We will not spend time discussing the details here.

However, detailed balance provides a constructive way to design transition probabilities. If we want samples from a target distribution $\pi(\mathbf{x})$, we need to construct a transition probability $T(\mathbf{x}, \mathbf{y})$ that satisfies
\begin{equation}
    \pi(\mathbf{x}) T(\mathbf{y}, \mathbf{x}) = \pi(\mathbf{y}) T(\mathbf{x}, \mathbf{y})
    \implies
...
\begin{theoremBox}
    \textbf{Heuristic: The 0.234 Rule.} A widely used rule of thumb (due to Roberts, Gelman, and Gilks 1997) holds that for random-walk Metropolis-Hastings with a well-tuned Gaussian proposal in $d$ dimensions, the proposal step size should shrink roughly like $d^{-1/2}$ as dimension grows, giving an optimal acceptance rate of approximately $0.234$ that is largely insensitive to the target's specific shape. This is an empirical/asymptotic guideline. Because the step size shrinks with $d$, the number of steps needed to traverse the target distribution grows with dimension even though each individual step stays computationally cheap. This is the reason for our earlier disclaimer that the \emph{constant} multiplying Monte Carlo's $M^{-1/2}$ rate grows with dimension. It's also part of why gradient-informed samplers, such as Langevin-based methods and Hamiltonian Monte Carlo (HMC), are the modern default for continuous, high-dimensional sampling problems.
\end{theoremBox}
...
In practice, \emph{parallel tempering} (also called replica exchange) is often preferred to this serial cooling schedule: rather than cooling a single chain over time, it runs multiple chains simultaneously across a range of temperatures and periodically proposes swaps between neighboring-temperature chains, letting a sample stuck in the cold (target) chain escape by hopping through a ``hot'' chain that mixes easily. This is the version of tempering actually implemented in most molecular simulation packages and Bayesian inference software. It is an area of active research.
...
Everything we have done so far uses Monte Carlo to sample from a \emph{static} distribution. ``Static'' refers to the fact that we draw samples from some target $\pi(\mathbf{x})$ to compute averages at equilibrium. But many problems in chemical engineering are inherently \emph{dynamic}. Reactions fire, molecules adsorb and desorb, crystals grow. The question is not ``what is the equilibrium state?'' but ``what happens next, and when?'' Kinetic Monte Carlo (kMC) methods answer this question by simulating the time evolution of a system one random event at a time.\footnote{As a fun fact, the earliest application of kMC to an atomistic system appears to have been \href{https://doi.org/10.1103/PhysRev.150.470}{Beeler's 1966 simulation of radiation damage annealing in iron}.}
% fun fact is from https://helper.ipam.ucla.edu/publications/matut/matut_5898_preprint.pdf

The setting is a system that can be in one of many discrete states $\mathbf{s}$ with $R$ possible events (reactions, transitions) that can occur. Each event $\mu$ has a \emph{propensity} $a_\mu(\mathbf{s})$ that depends on the current state and plays the role of a rate, which means that the probability that event $\mu$ fires in a small time interval $[t, t+dt)$ is $a_\mu \, dt$. The total propensity is $a_0 = \sum_{\mu=1}^{R} a_\mu$, and the time until the next event of \emph{any} kind is exponentially distributed:
\begin{equation}
    \tau \sim \text{Exp}(a_0), \quad \text{i.e.,} \quad p(\tau) = a_0 \, e^{-a_0 \tau}
\end{equation}
When that event fires, it is reaction $\mu$ with probability $a_\mu / a_0$. This is exact. It is mathematically equivalent to solving the chemical master equation, which tracks $p(\mathbf{s}, t)$ for every state $\mathbf{s}$, but without ever writing down or storing the (enormous) transition matrix!

\begin{algorithmBox}
    \textbf{The Gillespie (kMC) Algorithm}
    \begin{enumerate}
        \item Initialize the state $\mathbf{s}$ and set $t = 0$.
        \item Compute all propensities $a_\mu(\mathbf{s})$ for $\mu = 1, \ldots, R$ and their sum $a_0 = \sum_{\mu} a_\mu$.
        \item If $a_0 = 0$, stop (no more events can occur).
        \item Draw the waiting time: $\tau = -\ln(r_1) / a_0$, where $r_1 \sim \mathcal{U}(0,1)$.
        \item Select which event fires: find the $\mu$ satisfying $\sum_{\nu=1}^{\mu-1} a_\nu < r_2 \, a_0 \le \sum_{\nu=1}^{\mu} a_\nu$, where $r_2 \sim \mathcal{U}(0,1)$.
        \item Update the state $\mathbf{s}$ according to event $\mu$ and advance $t \to t + \tau$.
        \item Return to step 2.
    \end{enumerate}
\end{algorithmBox}

The algorithm presented here is usually called the Gillespie algorithm when applied to well-mixed chemical kinetics and kMC when applied to spatially resolved systems such as surface reactions on a lattice. The underlying mathematical framework (exact stochastic simulation of a continuous-time Markov chain) is the same in both cases.

The intuition behind this algorithm is a picture of competing clocks. Imagine each possible event $\mu$ has its own independent alarm clock that rings after a random, exponentially distributed time with rate $a_\mu$. All $R$ clocks are ticking simultaneously, and whatever rings first is the event that happens. The time to the first ring out of all $R$ clocks is exponential with rate $a_0 = \sum a_\mu$ (the more clocks ticking, the sooner one goes off), and the probability that it was clock $\mu$ that rang first is $a_\mu / a_0$. This is exactly what steps 4 and 5 sample. The property that makes this work is the \emph{memorylessness} property of the exponential distribution, the unique continuous distribution for which $P(\text{wait} > t + s \mid \text{wait} > t) = P(\text{wait} > s)$. After we fire an event and recompute the propensities, the remaining clocks don't ``remember'' how long they've been ticking, so we can throw them all away and start fresh. This is why the algorithm loops back to step 2 with no leftover state.

One subtlety that is easy to overlook is that the propensity $a_\mu$ must be expressed in terms of molecule counts, not concentrations. For a unimolecular reaction $A \to \cdots$ with deterministic rate $r = k[A]$, the stochastic propensity is simply $a = k N_A$ (units of $k$: s$^{-1}$). But for a bimolecular reaction $A + B \to \cdots$ with deterministic rate $r = k[A][B]$, the macroscopic rate constant $k$ (units of M$^{-1}$s$^{-1}$) must be converted to a stochastic rate constant $c = k / (N_{\text{Av}} V)$, giving propensity $a = c \, N_A N_B$, where $V$ is the system volume and $N_{\text{Av}}$ is Avogadro's number. Using the macroscopic $k$ directly would produce simulation timescales that are wrong by orders of magnitude.

As a concrete example, consider the consecutive reaction $A \xrightarrow{k_1} B \xrightarrow{k_2} C$. There are $R = 2$ possible events: the conversion of one $A$ molecule to $B$ (propensity $a_1 = k_1 N_A$) and the conversion of one $B$ molecule to $C$ (propensity $a_2 = k_2 N_B$). Because each event changes the molecule count by $\pm 1$, the trajectories are staircase functions. Figure~\ref{fig:kmc-trajectories} shows several stochastic realizations alongside the deterministic ODE solution $dN_A/dt = -k_1 N_A$, $dN_B/dt = k_1 N_A - k_2 N_B$. With only 50 molecules, individual trajectories fluctuate visibly around the deterministic curve. With 500 molecules, the relative fluctuations shrink (they scale as $1/\sqrt{N}$) and the stochastic trajectories collapse onto the ODE solution. This is the \emph{thermodynamic limit}, where deterministic kinetics emerge as a large-$N$ limit of the stochastic process.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=.98\textwidth]{figs/monte-carlo/kmc_trajectories.pdf}
    \caption{Kinetic Monte Carlo simulation of $A \to B \to C$ ($k_1 = 1$, $k_2 = 0.5$). Faint step functions are individual stochastic realizations; solid curves are the deterministic ODE solution. Left: with 50 molecules, fluctuations are significant. Right: with 500, the stochastic trajectories closely follow the deterministic limit.}
    \label{fig:kmc-trajectories}
\end{figure}

Because the algorithm fires one event at a time, its computational cost is proportional to the number of reaction events. This is fine when molecule counts are small, but becomes prohibitive for large populations or fast rates (e.g., $10^{23}$ molecules). Tau-leaping (\href{https://doi.org/10.1063/1.1378322}{Gillespie, 2001}) addresses this by leaping forward a larger time step $\Delta t$ and firing approximately $a_\mu \Delta t$ reactions per channel (drawn from Poisson distributions) rather than resolving each event individually. As $\Delta t$ grows, tau-leaping smoothly interpolates between the exact kMC and the deterministic ODE limit.

\newpage
\section{Exercises}

\begin{exercise} % 2020 Participation quiz L31
    Write a MATLAB script that uses rejection sampling to draw samples from the probability density function over \(\mathbf{x}\in\mathbb{R}^2\) with PMF: 
    \[
        p(\mathbf{x}) = \begin{cases}
            1/3, & x_1, x_2 \leq 1/2 \text{ or } x_1,x_2 > 1/2 \\
            1/6, & x_1 \leq 1/2, x_2 > 1/2 \text{ or } x_1 > 1/2, x_2 \leq 1/2\\
            0, & x_1,x_2 \notin [0,1]
        \end{cases}
    \]
    \begin{tikzpicture}[scale=4]
        \draw[->,thick] (-0.10,0) -- (1.20,0) node[below right]{$x_1$};
        \draw[->,thick] (0,-0.10) -- (0,1.20) node[above left] {$x_2$};
        \draw[blue,thick]        (0,0) rectangle (1,1);
        \draw[blue,thick,dashed] (0.5,0) -- (0.5,1);
        \draw[blue,thick,dashed] (0,0.5) -- (1,0.5);
        \node[blue] at (0.25,0.75){$\dfrac{1}{6}$};
        \node[blue] at (0.75,0.75){$\dfrac{1}{3}$};
        \node[blue] at (0.25,0.25){$\dfrac{1}{3}$};
        \node[blue] at (0.75,0.25){$\dfrac{1}{6}$};
    ...
    Plot a histogram of 1000 samples.
\end{exercise}

\begin{solution}
    \lstinputlisting{code/matlab/monte-carlo/L31_rejection.m}
\end{solution}

\begin{exercise} % 2020 Participation quiz L32
    For a target distribution: \(\pi(\mathbf{x})\propto \dfrac{1}{1+|\mathbf{x}|^2}\), with $\mathbf{x} \in \mathbb{R}^2$, write a MATLAB script that uses a Markov chain to draw samples. Plot a histogram of 1000 samples.
\end{exercise}

\begin{solution}
    \lstinputlisting{code/matlab/monte-carlo/L32_metropolis.m}

    \textbf{Note:} In two dimensions
    \[
        \int_{\mathbb{R}^2}\frac{d\mathbf{x}}{1+|\mathbf{x}|^{2}} = 2\pi\int_0^\infty\frac{r\,dr}{1+r^{2}}=\infty,
    \]
    so \(\pi\) as written is not normalizable. The chain drifts outward forever.
\end{solution}

%% TODO: reinstate once kMC written?

% \begin{exercise} % 2020 Participation quiz L33
%     Consider the system with \ce{I + I -> D} where the number of \ce{I} initially is 5 and the number of \ce{D} is 0.\\
%     Explain why the possible states, \(\mathbf{x}=(x_I, x_D)\), are \((5,0), (3,1), (1,2)\).\\
%     The chemical master equation (CME) for this system is
%     \begin{align*}
%         \frac{d}{dt}P((5,0),t|(5,0),0) &= c[-(5)(4)P((5,0),t|(5,0),0)] \\[4pt]
%         \frac{d}{dt}P((3,1),t|(5,0),0) &= c[(5)(4)P((5,0),t|(5,0),0) - (3)(2)P((3,1),t|(5,0),0)] \\[4pt]
%         \frac{d}{dt}P((1,2),t|(5,0),0) &= c[(3)(2)P((3,1),t|(5,0),0)]
%     \end{align*}
%     with initial conditions $P((5,0),0|(5,0),0) = 1,\, P((3,1),0|(5,0),0) = 0, \text{ and } P((1,2),0|(5,0),0) = 0$.
    
%     Write a MATLAB script to solve the CME with $c = 1$ and compute the average number of \ce{I} with time.
% \end{exercise}

% \begin{solution}
%     Each event \ce{I + I -> D} removes two \ce{I} and creates one \ce{D}, so after \(k\) events \(\mathbf{x}=(x_I,x_D)=(5-2k,\,k)\); from \((5,0)\) this gives \((5,0)\), \((3,1)\), \((1,2)\).

%     \lstinputlisting{code/matlab/monte-carlo/L33_cme.m}
% \end{solution}

\begin{exercise} % 2025 "Final extra problems" Nathan Morgan
    A Slater-type orbital is a PDF that is sometimes used to model atomic orbitals in computational chemistry.  One Slater-type orbital is described by
    \[
        R(x) = N\exp(-|x|),
    \]
    where $N$ is the normalization constant.  Analytically we know $N = 1/2$.

    \begin{figure}[h!]
        \centering
        \begin{tikzpicture}
        \begin{axis}[
            width=0.75\linewidth, height=0.35\linewidth,
            axis lines=middle,
            xmin=-6.2, xmax=6.2, ymin=-0.06, ymax=1.12,
            xtick={-6,-4,-2,2,4,6},
            ytick={0.5,1},
            grid=both,
            grid style={gray!25},
            minor tick num=1,
            samples=401, domain=-6:6,
            clip=false,
        ]
            \addplot[red, thick] {exp(-abs(x))};
        \end{axis}
        \end{tikzpicture}
    \end{figure}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item The following code aims to estimate $1/N$, but it consistently underestimates.  (The estimates are usually around $1.7$.)  Why?
        \lstinputlisting{code/matlab/monte-carlo/underestimates1.m}

        \item The following code aims to estimate $1/N$, but it
        consistently underestimates.  (The estimates are usually around
        $0.52$.)  Why?
        \lstinputlisting{code/matlab/monte-carlo/underestimates2.m}

        % \item Why did the previous two questions not use Markov Chain
        % Monte Carlo?

        \item Does the following code correctly sample points from $R(x)$?
        \lstinputlisting{code/matlab/monte-carlo/sampling.m}

        \item Assuming we have correctly sampled points from $R(x)$,
        can we use these samples to estimate $N$?  Explain.
...
\begin{solution}
    \begin{enumerate}[label=(\alph*)]
        \item This code attempts to estimate $1/N$ using the following scheme:
        \[
            \int_{\mathbb{R}} R(x)\mathrm{d}x = \int_{\mathbb{R}} N \exp(-|x|)\mathrm{d}x = 1
        \]
        \[
            1/N = \int_{\mathbb{R}} \exp(-|x|)\mathrm{d}x = \int_{\mathbb{R}} \frac{\exp(-|x|)}{p(x)}\cdot p(x)\mathrm{d}x
        \]
        We will draw samples from $p(x)$ and evaluate $\frac{\exp(-|x|)}{p(x)}$ at those points. Here samples are taken from $p(x) = \mathcal{U}(-2,2)$, but this doesn't cover all density of $R(x)$. So the contributions of the tails to the integral are lost.

        \item The estimate of $N$ is just the average value of $\exp(-|x|)$ over the samples, but we need to account for the probablility of our samples. We do this by taking the mean of $\frac{\exp(-|x|)}{p(x)}$ instead. Corrected code:
        \lstinputlisting{code/matlab/monte-carlo/corrected2.m}

        % \item MCMC lets us sample points from an arbitrary distribution. In the previous two parts we used \texttt{rand} and \texttt{randn} to sample points from the uniform and Gaussian distributions. We don't need MCMC to sample from these two distributions as we already have functions to do so.

        \item Yes.

        \item No.  The integral we are trying to estimate is
        $\int_{\mathbb{R}} \exp(-|x|)\,dx$.  This is not in the form
        $\int f(x)\,P(x)\,dx$ where $P(x)$ is a properly normalised PDF.
        If we try to recast our integral in this form,
        \[
            \int_{\mathbb{R}} \exp(-|x|)\,dx
            = \int_{\mathbb{R}} \frac{1}{N}\, N\exp(-|x|)\,dx
            = \int_{\mathbb{R}} \frac{1}{N}\, R(x)\,dx,
        \]
        then $f(x) = 1/N$.  We cannot evaluate this because we do not
        know $N$.

    \end{enumerate}
\end{solution}

\begin{exercise} % 2025 Review 2 Nathan Morgan
    Consider the following code:

    \lstinputlisting{code/matlab/monte-carlo/mcmc.m}

    Can we use this code to evaluate the integrals below? For each integral, answer whether it is possible: if so, specify how $f$ and $g$ should be defined; if not, explain why. As a bonus, also find the integral value by inspection of the integrand.

    \begin{enumerate}[label=(\alph*)]
        \item \(I = \int_{\mathbb{R}} x \cdot \dfrac{e^{-x^2}}{Z}\mathrm{d}x, \quad Z = \int_{\mathbb{R}} e^{-x^2}\mathrm{d}x\)\\[4pt]
        \item \(I = \int_{\mathbb{R}} \dfrac{e^{-x^2}}{Z}\mathrm{d}x, \quad Z = \int_{\mathbb{R}} e^{-x^2}\mathrm{d}x\)\\[4pt]
        \item \(I = \int_{\mathbb{R}} e^{-x^2}\mathrm{d}x\)
    \end{enumerate}
\end{exercise}

\begin{solution}
    \begin{enumerate}[label=(\alph*)]
        \item Yes: \(g(x) = e^{-x^2},\; f(x)=x\)\\
        \(g(x)\) doesn't need to be normalized for MCMC. The integral value is 0, due to symmetry of the integrand with respect to the bounds of integration.

        \item Yes: \(g(x) = e^{-x^2},\; f(x)=1\)\\
        The integral value is 1, as the integrand is a PDF.

        \item No---not as is. This integral is not in the form of an expectation value. But if we multiply and divide by a PDF, we can cast this integral as an expectation value.
        \[
            I = \int_\mathbb{R} \dfrac{e^{-x^2}}{p(x)} p(x)\mathrm{d}x
        \]
        The value of the integral is $\sqrt{\pi}$, as that is the inverse normalization of the standard gaussian distribution.

        As a side note, when choosing $p(x)$, the closer the shape of $p(x)$ matches $|e^{-x^2}|/\int|e^{-x^2}|dx$, the faster the MCMC approximation converges to the integral (i.e., it is better to sample points where our function value is high because those points contribute the most to the integral).
    \end{enumerate}
\end{solution}

\begin{exercise} % 2025 Review 2 Nathan Morgan
    In the Metropolis-Hastings algorithm, we first choose a proposal (transition) distribution $M(\mathbf{x}^*, \mathbf{x}_i)$, which gives the probability of proposing the candidate state $\mathbf{x}^*$ from the current state $\mathbf{x}_i$, and then construct the acceptance probability
    \[
        A(\mathbf{x}^*, \mathbf{x}_i) = \min \left\{ 1, \frac{\pi(\mathbf{x}^*)\, M(\mathbf{x}_i, \mathbf{x}^*)}{\pi(\mathbf{x}_i)\, M(\mathbf{x}^*, \mathbf{x}_i)} \right\},
    \]
    where $\pi$ is the target distribution. For each of the following choices of $M$, describe what issues we would encounter (if any) and any potential solutions.

    \begin{enumerate}[label=(\alph*)]
        \item $M$ is not symmetric, $M(\mathbf{x}^*, \mathbf{x}_i)\neq M(\mathbf{x}_i, \mathbf{x}^*)$
        % \item $M$ is one directional, $M(\mathbf{x}^*, \mathbf{x}_i) = 0.1 \text{ and } M(\mathbf{x}_i, \mathbf{x}^*)=0$
        \item $M$ is not "irreducible" (can't get to every other $\mathbf{x}^*$ from $\mathbf{x}_i$)
    \end{enumerate}
\end{exercise}

\begin{solution}
    \begin{enumerate}[label=(\alph*)]
        \item This isn't a problem, as by construction our $A$ will account for this asymmetry. If we use a symmetric $M$, then the M-H $A$ simplifies to 
        \[
            A(\mathbf{x}^*, \mathbf{x}_i) = \min \left\{ 1, \frac{\pi(\mathbf{x}^*)}{\pi(\mathbf{x}_i)} \right\}.
        \]

        % \item If $M=0$, we would get a divide by zero error when constructing $A$.

        \item We usually use an $M$ that only makes transitions in a local region (is not irreducible) like $\mathcal{U}(\mathbf{x}_i-c,\mathbf{x}_i+c)$ or $\mathcal{N}(\mathbf{x}_i,c)$. This leads to more accepted transitions and a smoother exploration of the surface. 
        
        In this case however, we are not guaranteed to converge to our target distribution because we only take local steps and there could be two regions with nonzero density separated by a region of zero density. One solution is to create multiple chains with different initializations.
    \end{enumerate}
\end{solution}
