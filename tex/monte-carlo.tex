...
\begin{exercise} % 2025 Review 2 Nathan Morgan
    For each of the integrals below, specify a functional form of $g$ and $f$ that can be used in the following code to approximate the integral value.
    
    \lstinputlisting{code/matlab/monte-carlo/mcmc.m}

    As a bonus, also find the integral value by inspection of the integrand. 
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item \(I = \int_{\mathbb{R}} e^{-x^2}\mathrm{d}x\) (This is a bit of a trick question, why?)
    \end{enumerate}
\end{exercise}

\begin{solution}
    \begin{enumerate}[label=(\alph*)]
        \item \(g(x) = e^{-x^2},\; f(x)=x\)\\
        ...
        \item \(g(x) = e^{-x^2},\; f(x)=1\)\\
        The integral value is 1, as the integrand is a PDF.
        
        \item This integral is not in the form of an expectation value. We can't use MCMC as is. But if we multiply and divide by a PDF, we can cast this integral as an expectation value.
        \[
...
\begin{exercise} % 2025 Review 2 Nathan Morgan
    In the Metropolis-Hastings algorithm, we first choose $M$ and then construct $A$. For each of the following choices of $M$, describe what issues we would encounter (if any) and any potential solutions. 
