...
\begin{exercise} % 2020 Participation quiz L31
    Write a \textsc{matlab} script that uses rejection sampling to draw samples from the probability density function over \(\mathbf{x}\in\mathbb{R}^2\) with PMF: 
    \[
...
\begin{exercise} % 2020 Participation quiz L32
    For a target distribution: \(\pi(\mathbf{x})\propto \dfrac{1}{1+|\mathbf{x}|^2}\), with $\mathbf{x} \in \mathbb{R}^2$, write a \textsc{matlab} script that uses a Markov chain to draw samples. Plot a histogram of 1000 samples.
\end{exercise}
...
\begin{exercise} % 2020 Participation quiz L33
    ...
    Write a \textsc{matlab} script to solve the CME with $c = 1$ and compute the average number of \ce{I} with time.
\end{exercise}
