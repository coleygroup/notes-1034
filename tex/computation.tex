...
There are various types of floating-point formats, determined by the number of bits allocated for the sign, exponent, and significand.
The most common floating-point formats are single-precision (float32) and double-precision (float64), which represent numbers with 32 and 64 bits, respectively (\autoref{fig:floating-point}).
\begin{figure}[!h]
  \centering
...
\[ \pi = 1.1001001000011111101101010\ldots \times 2^{1} \]
...
\begin{exercise}\label{ex:thomas-algorithm} % 2025 "Final extra problems" Nathan Morgan
  ...
  \]
  How does this algorithm scale with the size of the matrix? That is, is it $\mathcal{O}(n)$, $\mathcal{O}(n^2)$, $\mathcal{O}(n^3)$,
  $\mathcal{O}(n^4)$, etc.?
\end{exercise}

\begin{solution}
  We have three `for loops', each of roughly length $n$, so the scaling is $\mathcal{O}(n)$.
\end{solution}
