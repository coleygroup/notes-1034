...
\begin{solution}
The constant joint density implies probability is proportional to area. Taking the conditional $x_2 > 3$ means that we are only considering the portion of the shaded region above the horizontal line \(x_2=3\). That region is a simple rectangle where the probability density is (still) uniform. 

Thus \(_1\mid(X_2>3)\) is uniform on \((3,5)\) with density \(1/2\). \(\square\)
Thus, $p_\mathsf{x_1 \mid x_2}(x_1 | x_2 > 3)$ = \mathcal{U}[3,5]
\end{solution}
