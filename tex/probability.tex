...
\begin{exercise}
Suppose that we are going bird watching, and the number of birds we see per hour follows a Poisson distribution with rate parameter $\lambda = 3$.
What is the probability that we see at least one bird in one hour?
\end{exercise}
...
\begin{exercise}
Consider a 2D distribution that has constant probability density on the shaded region shown in the figure below and zero elsewhere.  Use a visual/geometric argument to find the conditional PDF, $p_\mathsf{x_1 \mid x_2}(x_1 | x_2 > 3)$. 

% --- figure: reproduction of the problem image ---
\begin{center}
...
%   % horizontal reference line x2 = 3
%   \draw[red, dashed, thick] (-0.1,3) -- (5.6,3) node[right,black] {\small $x_2=3$};
...
\begin{solution}
The constant joint density implies probability is proportional to area. Taking the conditional $x_2 > 3$ means that we are only considering the portion of the shaded region above the horizontal line \(x_2=3\). Only the top-right square, spanning $x_1\in[2.5,4]$ and $x_2\in[2.5,4]$, extends above $x_2=3$ (the other two squares top out at $x_2=2.5$ and $x_2=3$, respectively). The portion of that square with $x_2>3$ is the rectangle $x_1\in[2.5,4]$, $x_2\in(3,4]$, where the probability density is (still) uniform.

Thus, $p_\mathsf{x_1 \mid x_2}(x_1 | x_2 > 3) = \mathcal{U}[2.5,4]$.
\end{solution}
