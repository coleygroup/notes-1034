...
\begin{exercise} % 2025 Recitation slides 5 Nathan Morgan
    ...
    for some constant $c$. 
        \begin{enumerate}[label=(\alph*)]
            \item Find $c$ by normalizing the PDF.
            \item Find the cumulative distribution function $F_{\mathsf{x}}(x)$.
            \item Calculate the expected value $\mu_{\mathsf{x}} =
                \mathbb{E}[\mathsf{x}]$.
...
\begin{exercise} % 2025 Recitation slides 5 Nathan Morgan
    Let the random vector $\underline{\mathsf{x}}$ be defined by
    \[
        \underline{\mathsf{x}}(\underline{\xi})
    ...
    \]
    \begin{enumerate}[label=(\alph*)]
        \item Find the covariance matrix $\mathbf{C}$ of
              $\underline{\mathsf{x}}$.
        \item Using the result of (a), write the joint PDF
              $p_{\underline{\mathsf{x}}}(\mathbf{x})$ in simplified form.
        \item Find the marginal distribution $p_{\mathsf{x}_{2}}(x_{2})$
              by integrating $x_{1}$ out of the joint PDF.
        \item Rederive $p_{\mathsf{x}_{2}}(x_{2})$ using the conditional
              distribution $p_{\mathsf{x}_{2}|\mathsf{x}_{1}}(x_{2} \mid
              x_{1})$ together with $p_{\mathsf{x}_{1}}(x_{1})$.
    \end{enumerate}
\end{exercise}
...
\begin{solution}
    ...
    \textbf{(b)} Therefore $\underline{\mathsf{x}} \sim \mathcal{N}(\boldsymbol{\mu}, \boldsymbol{\Sigma})$ with $\boldsymbol{\mu} = \begin{bmatrix}0\\0\end{bmatrix}$ and $\boldsymbol{\Sigma} = \begin{bmatrix}1&1\\1&2\end{bmatrix}$.
    Applying the multivariate Gaussian PDF formula:
    \begin{multline*}
        p_{\underline{\mathsf{x}}}(\mathbf{x})
    ...
    \textbf{(d)} If $x_{1}$ is fixed, then $x_{2} = x_{1} + \xi_{2}$ has its mean shifted to $x_{1}$, so $x_{2} \mid x_{1} \sim \mathcal{N}(x_{1}, 1)$. Therefore
    \begin{align*}
        p_{\mathsf{x}_{2}}(x_{2})
        &= \int_{-\infty}^{\infty}
...
\begin{exercise} % 2025 Recitation slides 5 Nathan Morgan
    ...
    \[
        ...
        \end{cases}
    \]
    and let $p_{\mathsf{t}}(t)$ be a continuous PDF. Show that the convolution $(u\,{*}\,p_{\mathsf{t}})(t)$ equals the CDF $F_{\mathsf{t}}(t)$.
\end{exercise}
...
\begin{exercise}
    Calculate the covariance matrix for this set of measurements.
    \begin{table}[h!]
        \centering
        \begin{tabular}{c|cc}
            Measurement & $x_1$ & $x_2$ \\
