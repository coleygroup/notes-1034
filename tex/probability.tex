...
Note also that the covariance matrix is always symmetric ($C_{ij} = C_{ji}$) and positive semi-definite. The proof that a covariance matrix must be positive semi-definite connects back to our earlier discussions of linear algebra, so we will write it out here as a simple proof. Consider any non-zero vector $\mathbf{z} \in \mathbb{R}^N$. We can express the quadratic form $\mathbf{z}^\top \mathbf{C} \mathbf{z}$ as follows:
\begin{align}
