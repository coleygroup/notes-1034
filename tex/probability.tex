...
A very simple (and not very good!) example of a PRNG is the middle squares method. This method starts with a seed number, squares it, and then extracts the middle digits to form the next number in the sequence. This process is repeated to generate more numbers. For example, generating four-digit integers seeded with ``1034'', we might have:
\begin{equation*}
1034 ~ \rightarrow ~ 1034^2 = 1,069,156 ~ \rightarrow ~ \text{next number: } 0691 ~ \rightarrow ~ 0691^2 = 477,481 ~ \rightarrow ~ \text{next number: } 7748 ~ \rightarrow ~ \ldots
\end{equation*} 
...
\begin{align}
    D_{\mathrm{KL}}(p_{\mathsf{x}}\Vert q_{\mathsf{x}})= \mathbb{E}_{\mathsf{x} \sim p_{\mathsf{x}}(x)}\left[ \frac{p_{\mathsf{x}}(x)}{q_{\mathsf{x}}(x)} \right] &=  \int p_{\mathsf{x}}(x)\log\!\left(\frac{p_{\mathsf{x}}(x)}{q_{\mathsf{x}}(x)}\right)\,dx \text{ (continuous)} \\
...
\begin{exercise} % 2020 Participation quiz L26
    ...
    \textit{Reminder:} $\displaystyle P_k(k) = \frac{\lambda^k e^{-\lambda}}{k!}$
\end{exercise}

\begin{solution}
    Event ``at least one bird'' \(= \{k \ge 1\}\)
    \[
        \Rightarrow \mathbb{P}[k \ge 1] = 1 - \mathbb{P}[k=0] = 1 - P_{\mathsf{k}}(1) = 1 - \frac{\lambda^0 e^{-\lambda}}{0!} = 1 - e^{-3} = 0.95
    \]
\end{solution}
...
\begin{solution}
    ...
    \begin{tikzpicture}
        ...
        \node[red, right, align=left, font=\small] at (5.5,2.2) {
            $\mathbb{P}[x_2>3] = \text{hashed area} \cdot \dfrac{1}{5} = \dfrac{3}{10}$
        };
    \end{tikzpicture}

    A more formal argument based on the definition of conditional probability:
    \[
        \mathbb{P}[x_1 | x_2 > 3] = \frac{\mathbb{P}[x1, x2 > 3]}{\mathbb{P}[x_2>3]}
        = \begin{cases}
            0, & x_1 \notin [2.5,4]\\
            \frac{1/5}{\mathbb{P}[x_2>3]} = \frac{1}{5} \cdot \frac{10}{3} = \frac{2}{3}, & x_1 \in [2.5,4]
        \end{cases}
    \]
\end{solution}
...
\begin{solution}
    ...
    \begin{align*}
        P_{\text{post}}[p|\mathcal D] &= \begin{cases}
            0, & 0.5 \le p \le 1\\
            \frac{N+1}{2^{-N}}\cdot 2\cdot p^N, & \text{else} 
        \end{cases}\\
        &= \begin{cases}
            0, & 0.5 \le p \le 1\\
            (N+1)\cdot 2^{N+1}\cdot p^N, & \text{else} 
        \end{cases}\\
...
\begin{exercise}
    Consider the joint distribution $p(x_1, x_2 \mid a)$ given by the
    following table:
    \begin{center}
    \begin{tabular}{c|ccccc}
        $x_1 \;\backslash\; x_2$ & 1 & 2 & 3 & 4 & 5 \\\hline
        0 & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ \\
        1 & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ \\
    \end{tabular}
    \end{center}
    What is $p(x_1 \mid a = 3)$?
\end{exercise}

\begin{solution}
    To find the conditional distribution $p(x_1 \mid a = 3)$, extract the
    column corresponding to $x_2 = 3$ and normalise:
    \[
        p(x_1 \mid a = 3)
        = \frac{p(x_1,\; x_2 = 3)}{\displaystyle\sum_{x_1} p(x_1,\; x_2 = 3)}.
    \]
    Divide each entry in the $x_2 = 3$ column by the column sum to obtain the
