...
\begin{exercise} % 2025 Common mistakes on exam 1 Nathan Morgan
    ...
    \[
        ...
        \end{bmatrix}.
    \]
    The first-order necessary conditions for a constrained minimum require the existence of multipliers $\lambda_1, \lambda_2$ such that
    \[
        \nabla f(\mathbf{x}) = \lambda_1 \nabla c_1(\mathbf{x}) + \lambda_2 \nabla c_2(\mathbf{x}),
        \qquad
        \mathbf{c}(\mathbf{x}) = \mathbf{0}.
    \]
    For each of the following candidate points, determine whether it satisfies these first-order necessary conditions.
...
\begin{exercise} % 2025 Review 1 Nathan Morgan
    ...
    \begin{enumerate}[label=(\alph*)]
        \item Write the unconstrained minimization problem and explain why it has no finite solution.
        \item Add the constraint that the total moles must equal 10 and we need at least 5 moles of B. What is the solution? Which constraints are active at the optimum?
...
\begin{solution}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \]
        This linear objective function has no finite minimum because both variables can be decreased indefinitely without bound. As $n_A \to -\infty$ or $n_B \to -\infty$, the objective approaches $-\infty$.
        
        \item B costs the least, so we want to maximize it. The maximum value of $n_B$ given the equality constraint is $n_B = 10$, so the optimum is $n_A = 0$, $n_B = 10$. The equality constraint is active, but the inequality constraint $n_B \geq 5$ is inactive.
...
\begin{exercise} % 2025 Review 1 Nathan Morgan
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item Explain why minimizing this error norm is unnecessarily complicated compared to solving the problem directly with linear algebra.
    \end{enumerate}
\end{exercise}
...
\begin{solution}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item This approach is unnecessarily complicated because the constraints form a linear system with a unique solution. Instead of iterative optimization (which requires initial guesses, gradient computations, and convergence checks), we can solve the system directly:
        \[
            \begin{bmatrix} 1 & 1 \\ 1 & -2 \end{bmatrix} \begin{bmatrix} v_1 \\ v_2 \end{bmatrix} = \begin{bmatrix} 3 \\ 0 \end{bmatrix}
        \]
        Using linear algebra, we immediately obtain $v_1 = 2$ L/s and $v_2 = 1$ L/s without the overhead of optimization.
    \end{enumerate}
\end{solution}

\begin{exercise} % 2025 Review 1 Nathan Morgan
    (*) Consider optimization algorithms with step updates of the form $\mathbf{x}_{i+1} = \mathbf{x}_i + \alpha d$ where $d$ is a descent direction.
    \begin{enumerate}[label=(\alph*)]
        \item For steepest descent with $d = -\nabla f(\mathbf{x}_i)$, derive the optimal step size $\alpha_{\text{opt}}$ by minimizing a second-order Taylor expansion of $f(\mathbf{x}_i + \alpha d)$.
        \item Repeat part (a) for Newton's method where $d = -(\nabla^2 f(\mathbf{x}_i))^{-1}\nabla f(\mathbf{x}_i)$. What is the optimal step size?
    \end{enumerate}
\end{exercise}

\begin{solution}
    \begin{enumerate}[label=(\alph*)]
        \item Using a second-order Taylor expansion:
        \[
            f(\mathbf{x}_i + \alpha d) \approx f(\mathbf{x}_i) + \alpha \nabla f(\mathbf{x}_i)^\top d + \tfrac{1}{2}\alpha^2 d^\top \nabla^2 f(\mathbf{x}_i) d
        \]
        With $d = -\nabla f(\mathbf{x}_i)$, this becomes:
        \[
            \phi(\alpha) = f(\mathbf{x}_i) - \alpha \|\nabla f(\mathbf{x}_i)\|_2^2 + \tfrac{1}{2}\alpha^2 \nabla f(\mathbf{x}_i)^\top \nabla^2 f(\mathbf{x}_i) \nabla f(\mathbf{x}_i)
        \]
        ...
        \[
            \phi(\alpha) = f(\mathbf{x}_i) - \alpha \mathbf{g}^\top \mathbf{H}^{-1}\mathbf{g} + \tfrac{1}{2}\alpha^2 \mathbf{g}^\top (\mathbf{H}^{-1})^\top \mathbf{H} \mathbf{H}^{-1} \mathbf{g}
        \]
        Since $\mathbf{H}$ is symmetric, $(\mathbf{H}^{-1})^\top = \mathbf{H}^{-1}$, so:
        \[
            \phi(\alpha) = f(\mathbf{x}_i) - \alpha \mathbf{g}^\top \mathbf{H}^{-1}\mathbf{g} + \tfrac{1}{2}\alpha^2 \mathbf{g}^\top \mathbf{H}^{-1}\mathbf{g}
        \]
        Minimizing:
        \[
            \frac{d\phi}{d\alpha} = -\mathbf{g}^\top \mathbf{H}^{-1}\mathbf{g} + \alpha \mathbf{g}^\top \mathbf{H}^{-1}\mathbf{g} = 0
        \]
...
\begin{exercise} % 2025 Review 1 Nathan Morgan
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item If our function is quadratic (e.g. $f(x) = x_1^2 + 10x_2^2$), using Newton's method will get us to the minimum in a single step.
        \item We don't need to use Hessian modification if we use the BFGS method to estimate the Hessian.
        \item If our function is quadratic (e.g. $f(x) = x_1^2 + 10x_2^2$), using the steepest descent algorithm with the optimal step size given by a quadratic approximation, will get us to the minimum in a single step.
    \end{enumerate}
\end{exercise}
