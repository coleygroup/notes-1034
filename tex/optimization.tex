...
\begin{exercise} % 2025 Review 1 Nathan Morgan
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item If the objective function is quadratic (e.g. $f(x) = x_1^2 + 10x_2^2$), using Newton's method will get us to the minimum in a single step.
        \item If the objective function is quadratic (e.g. $f(x) = x_1^2 + 10x_2^2$), using the steepest descent algorithm with the optimal step size given by a quadratic approximation, will get us to the minimum in a single step.
...
\begin{solution}
    \begin{enumerate}[label=(\alph*)]
        ...
        \item True if the function is convex. Newton's method gets us to a stationary point, which may be a maximum or saddle point instead.

        \item False, the quadratic approximation for optimal alpha is only searching in the direction of the negative gradient, which may not be pointing at the minimum.
