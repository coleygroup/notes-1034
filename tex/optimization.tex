...
\begin{solution}
    When \(\lambda = 0\), the only root of \(\lambda x^2 + e^{-x^2}\) is \(x = 0\):
    \begin{align*}
        0x^2 + e^{-x^2} - 1 &= 0\\
        e^{-x^2} - 1 &= 0\\
        e^{-x^2} &= 1\\
...
\begin{solution}
    A defining characteristic of constrained optimization is that the solution is at a tangent point between a contour and the constrain. The smallest contour in contact with the constraint is \(f(\mathbf{x})=2\), and its tangent point to the constraint is shown in blue.
